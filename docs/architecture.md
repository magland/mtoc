...
- **Function handles** (`src/lowering/lowerHandle.ts`): `@name` and
  ...
  `HandleType` carrying the resolved target identity (user function
  AST + file, or builtin name, or synthesized anonymous AST) PLUS any
  captures collected from the enclosing scope at the `@(...)` site.
  Handles are a new owned kind: per-shape typedefs and helpers are
  generated by `src/codegen/emitHandle.ts` (mirroring `emitStruct.ts`)
  and registered in `ownedKinds` so the existing declaration / free /
  copy / assign machinery picks them up. The function-call DISPATCH
  stays static: a higher-order function like `apply(h, x)` specializes
  per-target AND per-capture-shape via `canonicalizeType`, and each
  `h(args)` call site reads the handle's MType, peels off the target,
  and emits a direct call to the resolved specialization — no
  function-pointer table, no dispatcher. The captures' VALUES ride
  in the struct, fed back into the underlying call via
  `HandleCaptureLoad` IR nodes. Anonymous functions are synthesized
  as `FunctionStmt`s with `[...userParams, ...captureNames]` as
  params and routed through the existing `specializeUserCall`
  pipeline. See `docs/limitations.md` for the deferred surface
  (branch-divergent handles, handles in tensors/cells, `feval`).
...
- `IRExpr` — `NumLit`, `Var`, `TensorLit`, `Call`, `Binary`, `Unary`,
  `HandleLit`, `HandleCaptureLoad`, .... `Call` carries a
  `callee: CallTarget` discriminator (`builtin` / `userFunc`) —
  codegen no longer does string lookups against the runtime registry.
  `HandleLit` carries the captured values + resolved target on its
  `ty: HandleType`; codegen renders it as a C compound literal of the
  per-shape handle typedef. `HandleCaptureLoad` reads a single capture
  out of a handle struct (`<handle.cName>.cap_<name>`); used by
  `lowerHandleCall` to feed captures into the underlying
  specialization at each call site.
...
- `dispKinds.ts` — registry mapping each value shape (text / tensor /
  ...
  cell / struct / class disp lowering plugs in by adding one entry.
